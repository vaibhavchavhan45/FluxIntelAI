...
def handle_video_state(videoId: str, languages: str, background_tasks: BackgroundTasks, isRetry: bool = False, title: str = "", user_id: str = ""):
    """
    ...
    if state_response and state_response["status"] == "FAILED":
        ...
        if isRetry:
            ...
            set_video_state(videoId, status="PROCESSING", languages=languages, error=None)
            background_tasks.add_task(process_video_service, videoId, languages, title, user_id)
            return {
                "status": "PROCESSING",
    ...
    # READY state
    if title:
        set_video_state(videoId, "READY", languages, title=title)
    return None