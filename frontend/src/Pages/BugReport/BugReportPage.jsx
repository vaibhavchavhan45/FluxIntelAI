import { useNavigate } from "react-router-dom";
import Navbar from "../../Components/Navbar";
import Footer from "../../Components/Footer";
import BugReportHero from "./BugReportHero";
import BugReportForm from "./BugReportForm";
import BugThankYouCard from "./BugThankYouCard";
import useBugReport from "../../Hooks/BugReport/useBugReport";
import { githubIssueUrl } from "../../Data/BugReport/bugReportData";
import "../../Styles/feedback.css";

function BugReportPage() {
  const navigate = useNavigate();

  const {
    formRef,
    thankYouRef,
    submitted,
    setSubmitted,
    selectedSeverity,
    setSelectedSeverity,
    isSubmitting,
    bugFormErrorMsg,
    handleClear,
    handleSubmit,
  } = useBugReport();

  const links = [
    { label: "Home", onClick: function () { navigate("/"); } },
    { label: "About", onClick: function () { navigate("/about"); } },
    { label: "Feedback", onClick: function () { navigate("/feedback"); } },
    { label: "Docs", onClick: function () { } },
  ];

  const footerColumns = [
    {
      heading: "Studio",
      links: [
        { label: "Home", action: function () { navigate("/"); window.scrollTo(0, 0); } },
        { label: "About", action: function () { navigate("/about"); window.scrollTo(0, 0); } },
        { label: "Feedback", action: function () { navigate("/feedback"); window.scrollTo(0, 0); } },
      ],
    },
    {
      heading: "Legal",
      links: [
        { label: "Privacy Policy", action: function () { navigate("/privacy-policy"); window.scrollTo(0, 0); } },
        { label: "Terms of Service", action: function () { navigate("/terms-conditions"); window.scrollTo(0, 0); } },
      ],
    },
  ];

  const footerBottomRight = (
    <button
      onClick={function () { navigate("/"); window.scrollTo(0, 0); }}
      className="px-5 py-2 rounded-full border border-neutral-600 text-neutral-300 text-xs tracking-widest uppercase hover:bg-red-600 hover:border-red-600 hover:text-white cursor-pointer transition-all duration-300"
    >
      ← Back to Home
    </button>
  );

  return (
    <div className="min-h-screen bg-gradient-to-br from-[#f5f3f8] via-[#f1edf6] to-[#ece7f4] text-neutral-900">

      <Navbar links={links} onGetStarted={function () { navigate("/app"); }} />

      <BugReportHero submitted={submitted} />

      <div className="max-w-[52rem] mx-auto px-4 sm:px-6 pb-32 mt-10">
        {submitted ? (
          <BugThankYouCard
            thankYouRef={thankYouRef}
            onSubmitAnother={function () { setSubmitted(false); handleClear(); }}
          />
        ) : (
          <>
            <BugReportForm
              formRef={formRef}
              selectedSeverity={selectedSeverity}
              setSelectedSeverity={setSelectedSeverity}
              isSubmitting={isSubmitting}
              bugFormErrorMsg={bugFormErrorMsg}
              handleSubmit={handleSubmit}
              handleClear={handleClear}
            />

            {/* FOR DEVELOPERS */}
            <div className="relative top-15 flex flex-col items-center gap-3 mt-10">
              <p className="text-xs text-neutral-400 tracking-wide">Are you a developer?</p>
              <button
                onClick={function () { window.open(githubIssueUrl, "_blank"); }}
                className="inline-flex items-center gap-2.5 px-6 py-2.5 rounded-full bg-neutral-900 text-white text-xs font-semibold cursor-pointer"
              >
                <span className="text-base">⚠️</span>
                Report issue on GitHub
                <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="white">
                  <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
                </svg>
              </button>
            </div>
          </>
        )}
      </div>
        <Footer linkColumns={footerColumns} bottomRight={footerBottomRight} />
    </div>
  );
}

export default BugReportPage;